...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
      <h3 class="panel-title"><strong>CLASIFICACION DE MUESTRAS DE TPHA </strong></h3>
    </div>
    <div class="panel-body">
      ...
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          ...
          <p class="text-right" style="margin: 0 0 0px;"><b>Acciones</b>: <button class="btn btn-success btn-xs"><i class="glyphicon glyphicon-eye-open"></i></button>=Imprimir planilla de envío | <span style="color: #449d44;"><i class="glyphicon glyphicon-ok"></i></span>=Recepcionar Envío</p>
      ...
      <div class="panel panel-info">
        ...
        <div class="panel-body">
          <p class="text-right" style="margin: 0 0 0px;"><b>Acciones</b>: <button class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-list"></i></button>=Ver detalle de muestras <span class="text-danger"><b>rechazadas</b></span> | <button class="btn btn-success btn-xs"><i class="glyphicon glyphicon-eye-open"></i></button>=Imprimir planilla de envío | <span style="color: #449d44;"><i class="glyphicon glyphicon-ok"></i></span>=Clasificar muestras pendientes de recepción</p>
      ...
      <div class="panel panel-success">
        ...
        <div class="panel-body">
          <p class="text-right" style="margin: 0 0 0px;"><b>Acciones</b>: <button class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-list"></i></button>=Ver detalle de muestras <span class="text-danger"><b>rechazadas</b></span> | <button class="btn btn-success btn-xs"><i class="glyphicon glyphicon-eye-open"></i></button>=Imprimir planilla de envío | <button class="btn btn-warning btn-xs"><i class="glyphicon glyphicon-print"></i></button>=Imprimir resultados</p>
  ...
  <div class="modal fade" id="showDetObsModal" role="dialog" aria-labelledby="showDetObsModalLabel">
	<div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="showDetObsModalLabel">Detalle de mustras rechazadas</h4>
        </div>
        <div class="modal-body">
			<div id="div_detalle_rechazado"></div>
		</div>
		<div class="modal-footer">
  ...
  <script Language="JavaScript">
    ...
    function open_detobs(id_envio) {
      $.ajax({
        url: "../../controller/ctrlLab.php",
        type: "GET",
        data: {
          accion: 'GET_SHOW_DETALLERECHAZOMUESTRA', id_envio: id_envio
        },
        success: function (registro) {
			$('#div_detalle_rechazado').html(registro);
  ...
  function print_validados(idEnv) {
    var urlwindow = "pdf_laboratorio_psa.php?id_envio=" + idEnv;
    day = new Date();
    id = day.getTime();
    Xpos = (screen.width / 2) - 390;
    Ypos = (screen.height / 2) - 300;
  ...
  function reg_recepcion_manual(txtIdEnv, nomDep, nroEnv) {
    bootbox.confirm({
      message: "Se recepcionará el Envio: <b>" + nroEnv + " de " + nomDep + "</b>, ¿Está seguro de continuar?",
      buttons: {
        confirm: {
          label: 'Si',
          className: 'btn-success'
      ...
      callback: function (result) {
        if (result == true) {
          ...
          $.ajax( {
            ...
            url: '../../controller/ctrlLab.php',
            data: {
              accion: 'POST_RECEPCION_ENVIO_EXAMEN', id_envio: txtIdEnv,
              rand: myRand,
            },
            success: function(data) {
              var tmsg = data.substring(0, 2);
              ...
              if(tmsg == "OK"){
                ...
                window.location = './main_tpharegrecepcion.php?envivo=' +  txtIdEnv;
              } else {
                bootbox.alert(msg);
              }
            }
  ...
  $(document).ready(function () {
    ...
    var dTable = $('#tblSolicitud').DataTable({
      ...
      "bInfo": true,
      "bFilter": false,
      "sAjaxSource": "tbl_principalsolirecepPenTPHA.php", // Load Data
      "language": {
    ...
    var pTable = $('#tblEnProceso').DataTable({
      ...
      "fnServerParams": function (aoData)
      {
		...
		aoData.push({"name": "opt", "value": 'LAB'});
      },
