...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
		<h3 class="panel-title"><strong>REGISTRO DE RESULTADO</strong></h3>
    </div>
        ...
        <input type="hidden" name="id_producto_detalle" id="id_producto_detalle" value=""/>
          ...
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
			  ...
			  <li class=""><a href="#timeline" data-toggle="tab" aria-expanded="false"><b><span  class="text-green" >Por validar</span></b></a></li>
            </ul>
					...
					<table class="table table-bordered">
						<thead>
						  <tr>
							...
							<th><small>Adjuntar</small></th>
							<th><small>Procesar</small></th>
						  </tr>
						</thead>
						<tbody>
							<tr>
								<td><b class="text-info">Descargar plantilla (Archivo Excel)</b></td>
								<td>
									<button class="btn btn-primary btn-sm btn-flat" type="button" id="BtnDescargaPlantilla" onclick="descargar_archivo();"><i class="fa fa-download"></i> Descargar</button>
								</td>
								<td>
								</td>
							</tr>
							<tr>
								<td><b class="text-info">Cargar resultados desde Excel</b></td>
								<td>
									<input type="file" name="sortpic1" id="sortpicture1" onclick="activa_validar('1')">
								</td>
								<td>
									<button class="btn btn-success btn-sm btn-block btn-flat" type="button" id="BtnProcesar1" onclick="procesar_archivo('1');" disabled><i class="glyphicon glyphicon-compressed"></i> Procesar</button>
								</td>
							</tr>
						</tbody>
					</table>
					<table id="tblAceptable" class="display" cellspacing="0" width="100%">
					  <thead class="bg-aqua">
						<tr>
						  ...
						  <th>Establecimiento procedencia</th>
						  <th>N° Doc.</th>
						  <th>Nombre del paciente</th>
						  <th>Nro. envío</th>
						  <th>Fecha envío</th>
						  <th>Nro. Atención<br/>EESS Procedencia</th>
					...
					<table id="example" class="display" cellspacing="0" width="100%">
					  <thead class="bg-aqua">
						<tr>
						  ...
						  <th>N° Doc.</th>
						  <th>Nombre del paciente del EESS Procedencia</th>
						  <th>Nro. envío</th>
						  <th>Fecha envío</th>
						  <th>Nro. Atención<br/>EESS Procedencia</th>
						  <th>Códico<br/>Atención</th>
  ...
  <div class="modal fade" id="showIngResulModal" role="dialog" aria-labelledby="showIngResulModalLabel">
	...
	function openFormIngResul(id_atencion, dni) {
        ...
        $('#txt_codigo_ref_lab').val('');
		$('#txt_det_resultado').val('');
		$('#txt_fecha_resultado').val('');
		$('#txt_codigo_ref_lab').prop("disabled", false);
		$('#txt_det_resultado').prop("disabled", false);
		$('#txt_fecha_resultado').prop("disabled", false);
		...
		$('#showIngResulModal').on('shown.bs.modal', function (e) {
			$("#txt_codigo_ref_lab").trigger('focus');
		})
    }
	...
	function openFormIngResulValid(id_atencion, dni, codref, resul, fec) {
		$('#txt_opcion').val('VR');
		$('#txt_id_atencion').val(id_atencion);
		$('#showIngResulModalLabel').text(dni);
        $('#txt_codigo_ref_lab').val(codref);
		$('#txt_det_resultado').val(resul);
		$('#txt_fecha_resultado').val(fec);
    ...
    $(document).ready(function () {
      ...
      var aTable = $('#tblAceptable').DataTable({
        ...
        "columnDefs": [
          ...
          {"orderable": true, "targets": 1, "searchable": true, "class": ""},
      ...
      var table = $('#example').DataTable({
        ...
        'columnDefs': [
          ...
          {"orderable": false, "targets": 6, "searchable": false, "class": "text-center"},
          {"orderable": false, "targets": 7, "searchable": false, "class": "text-center"},
          {"orderable": true, "targets": 8, "searchable": false, "class": "font-weit text-center"},
		  {"orderable": false, "targets": 9, "searchable": false, "class": "font-weit text-right"},
