...
$rResult = $lab->get_tblDatosIngResultadoTPHA($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
...
foreach ($rResult as $aRow) {
	...
	if($input['id_estado'] == "1"){
		...
		$row = array($aRow['abrev_tipodocpac'] . ": " . $aRow['nro_docpac'], $aRow['nombre_rspac'], $aRow['nro_hc'], $aRow['edad_pac'], $aRow['cod_ref_nro_atencion'], $btnDet.$btnIngResul);
	} else {
		$idcod = $aRow['id_resuldet'];
		$btnIngResul = ' <a href="#" data-target="#editProductModal" class="acept" data-toggle="tooltip" data-placement="top" title="Validar y agregar comentario" onclick="openFormIngResulValid(\'' . $aRow['id_resuldet'] . '\',\'' . $aRow['abrev_tipodocpac'] . ": " . $aRow['nro_docpac'] . '\',\'' . $aRow['cod_ref_nro_atencion'] . '\',\'' . $aRow['det_resul'] . '\');"><i class="glyphicon glyphicon-ok"></i><i class="glyphicon glyphicon-ok"></i></a>';
		$row = array($idcod, $aRow['abrev_tipodocpac'] . ": " . $aRow['nro_docpac'], $aRow['nombre_rspac'], $aRow['det_resul'], $aRow['nro_hc'], $aRow['edad_pac'], $aRow['cod_ref_nro_atencion'], $btnIngResul);		
	}
    $output['aaData'][] = $row;
}
