...
<div class="container-fluid">
          ...
          <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
              ...
              <div id="settings" class="tab-pane">
                <p class="text-right" style="margin: 0 0 0px;"><b>Acciones</b>:  <span style="color: #F44336;"><i class="glyphicon glyphicon-remove"></i></span>=Cambiar a RECHAZADA</p>
            ...
            <div class="form-group">
              <div class="row">
                <div class="col-sm-12">
                  ...
                  <select class="form-control" name="txtIdObsEstEnvDet" id="txtIdObsEstEnvDet" style="width: 100%"></select>
                </div>
                <div class="col-sm-12">
  ...
  <!-- Modal donde se ingresa el motivo del rechazo-->
  <div class="modal fade" id="showRechazaModal" role="dialog" aria-labelledby="showRechazaModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="showRechazaModalLabel"></h4>
        </div>
        <div class="modal-body">
          <form name="frm_rechazo" id="frm_rechazo">
            <div class="form-group">
              <div class="row">
                <div class="col-sm-12">
                  <label for="txt_id_motivo_rechazo">Motivo:</label>
                  <select class="form-control" name="txt_id_motivo_rechazo" id="txt_id_motivo_rechazo" style="width: 100%"></select>
                </div>
                <div class="col-sm-12">
                  <label for="txt_motivo_det_rechazo">Detalle Motivo:</label>
                  <textarea class="form-control" name="txt_motivo_det_rechazo" id="txt_motivo_det_rechazo" rows="3"></textarea>
                </div>
              </div>
            </div>
          </form>
        ...
        <div class="modal-footer">
          <div class="row">
            <div class="col-md-12 text-center">
              <div class="btn-group">
                <button type="button" class="btn btn-danger btn-continuar" id="btn_valid_rechazo" onclick="save_form_rechazar()"><i class="fa fa-save"></i> Cambiar a RECHAZADO </button>
                <button type="button" class="btn btn-default" data-dismiss="modal"><i class="glyphicon glyphicon-remove"></i> Cancelar</button>
              </div>
            </div>
          </div>
  ...
  <div class="modal fade" id="showDetObsModal" role="dialog" aria-labelledby="showDetObsModalLabel">
    ...
    function get_listaobsdetenv(id_producto, opt) {//idproducto(60): PSA; opt(4):Rechazado
		$.ajax({
			...
			success: function (result) {
				...
				$('#txt_id_motivo_rechazo').html(newOption);
			}
      });
    }
        ...
        callback: function (result) {
          if (result == true) {
            ...
            $.ajax( {
              ...
              url: '../../controller/ctrlLab.php',
              data: {
                accion: 'POST_ADD_REGENVIOOBSOACEPTARDET', txtTipoOpe: 'AO', id_envio: txtIdEnv, txtIdEnvDet: txtIdEnvDet, txtIdEstEnvDet: '', txtIdObsEstEnvDet: '', txtDetObsEstEnvDet: document.frmSubsanar.txtDetSubEstEnvDet.value,
                rand: myRand,
              },
              success: function(data) {
                var tmsg = data.substring(0, 2);
   ...
   function open_rechazar(idenvdet, nrolam) {
    ...
    function save_form_rechazar() {
      $('#btn_valid_rechazo').prop("disabled", true);

      var idmotobs = document.frm_rechazo.txt_id_motivo_rechazo.value;
      if(idmotobs == ""){
        ...
        callback: function (result) {
          if (result == true) {
            ...
            $.ajax( {
              ...
              url: '../../controller/ctrlLab.php',
              data: {
                accion: 'POST_ADD_REGENVIOOBSOACEPTARDET', txtTipoOpe: 'RO', id_envio: txtIdEnv, txtIdEnvDet: txtIdEnvDet, txtIdEstEnvDet: '4', txtIdObsEstEnvDet: document.frm_rechazo.txt_id_motivo_rechazo.value, txtDetObsEstEnvDet: document.frm_rechazo.txt_motivo_det_rechazo.value,
                rand: myRand,
              },
              success: function(data) {
                var tmsg = data.substring(0, 2);
                ...
                if(tmsg == "OK"){
                  ...
                  bootbox.alert(msg);
				  $('#btn_valid_rechazo').prop("disabled", false);
                  return false;
                }
              }
            });
...
/***********************************************************************************************************************************/
    ...
    function open_rechazar(idenvdet, nrolam) {
      document.frmPrincipal.txtIdEnvDet.value = idenvdet;
      $('#showRechazaModal').modal({
        show: true,
        backdrop: 'static',
        focus: true,
      });

      $('#showRechazaModal').on('shown.bs.modal', function (e) {
        var modal = $(this)
        modal.find('.modal-title').text('Cambiar a RECHAZADO a la muestra del paciente: ' + nrolam)
		$("#txt_id_motivo_rechazo").val('').trigger("change");
        })
    }
