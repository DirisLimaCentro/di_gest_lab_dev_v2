...
$rResult = $lab->get_tblDatosProductoEnviadosAceptadas($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$recordsFiltered = 0;
// si viene cant_rows en el primer registro
...
foreach ($rResult as $aRow) {
    ...
    $btnRecha = '<a href="#" class="delete" data-toggle="tooltip" title="Rechazar" onclick="event.preventDefault(); event.stopPropagation(); abrirModalRechazo(\'' . $aRow['id'] . '\');"><i class="fa fa-ban"></i></a>';
    //$btnEnvOri = '<a href="#" class="detail" data-toggle="tooltip" title="Rechazar" onclick="event.preventDefault(); event.stopPropagation(); envia_datos_reg_orion(\'' . $aRow['id'] . '\');"><i class="fa fa-paper-plane"></i></a>';
    $btnEnvOri = '<a href="#" onclick="verDetalleAtencion(\'' . $aRow['id'] . '\')"> <i class="fa fa-eye"></i></a>';


    $output['aaData'][] = array(
        "nro_atencion"  => $aRow['nro_atencion'],
        "paciente"      => "<b>" . $aRow['nombre_paciente'] . "</b><br>" . $aRow['abrev_tipodoc'] . ": " . $aRow['nrodoc'],
        "dependencia_origen"   => $aRow['dependencia_origen'],
        "fecha_toma_muestra"=> $aRow['fecha_toma_muestra'],
        "fecha_envio_destino"    => $aRow['fecha_envio_destino'],
        "fecha_recibe_destino"  => $aRow['fecha_recibe_destino'],
        "id_estado_resul"        => $aRow['id_estado_resul'],
        "estado_resul"        => $aRow['estado_resul'],
        "btn_rechazar"  => $btnRecha, 
        "btn_envio_ori"  => $btnEnvOri
    );
}
