...
$rResult = $lab->get_tblDatosProductoEnviadosObservadas($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$recordsFiltered = 0;
...
foreach ($rResult as $aRow) {
    ...
    $btnAcet = '<a href="#" data-target="#editProductModal" class="acept" data-toggle="tooltip" data-placement="top" title="Ver detalle" onclick="event.preventDefault(); event.stopPropagation(); aceptar_muestra_desde_obs(\'' . $aRow['id'] . '\'); "><i class="fa fa-check"></i></a>';

    $output['aaData'][] = array(
        "nro_atencion" => $aRow['nro_atencion'],
        "paciente" => $aRow['nombre_paciente'] . "<br>" . $aRow['abrev_tipodoc'] . ": " . $aRow['nrodoc'],
