...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
      <h3 class="panel-title"><strong>REGISTRO DE ENVÍO DE MUESTRAS A LABORATORIO REFERENCIAL</strong></h3>
    </div>
    <div class="panel-body">
      <form class="" id="frm-example" method="POST">
        <input type="hidden" name="txtIdSolicitud" id="txtIdSolicitud" value="<?php echo $_GET['nroSoli']?>"/>
            <div class="row">
              <div class="col-sm-4">
                <b>EESS del laborarotio a enviar</b>
                <?php $rsD = $d->get_listaDependenciaConLaboratorio(); ?>
                <select name="txtIdDepRef" id="txtIdDepRef" style="width:100%;" class="form-control input-sm"  onkeydown="campoSiguiente('btnBus', event);">
			  ...
			  <div class="col-sm-8">
				<p class="text-right" style="margin: 0 0 0px;"><b>Acciones</b>: <input type="checkbox" checked>=Incluir en el envío | <input type="checkbox">=No incluir en el envío | <a style="color: #00c0ef;"><i class="glyphicon glyphicon-eye-open"></i></a>=Ver ficha de solicitud</p>
			  </div>
            </div>
      <table id="example" class="display" cellspacing="0" width="100%">
        <thead class="bg-aqua">
          <tr>
            <th></th>
            <th>N° Reg.</br>Laboratorio</th>
            <th>N°</br>Solicitud</th>
            <th>Paciente</th>
            <th>N° Doc.</th>
            <th>HC</th>
            <th>Establecimiento Pertenece</th>
...
function back() {
  window.location = './main_principallabenv.php';
}
...
function validForm() {
  ...
  if($("#txtIdDepRef").val() == "0"){
    msg+= "Seleccione un Establecimiento que cuenta con Laboratorio<br/>";
    sw = false;
  }
  ...
  if(rows_selected.join(",") == ""){
    msg+= "Seleccione al menos una solicitud<br/>";
    sw = false;
  }
    ...
    callback: function (result) {
      if (result == true) {
        ...
        $.ajax( {
          ...
          data: {
            accion: 'POST_ADD_REGENVIOLABREF',
			id_dependencia_destino: $('#txtIdDepRef').val(),
            id_atencion: $('#example-console-rows').text(),
            rand: myRand,
          },
...
$(document).ready(function () {
  ...
  var table = $('#example').DataTable({
    "lengthMenu": [[50, 100 ,250], [50, 100 ,250, "All"]],
      ...
      "bInfo": true,
      "bFilter": false,
    "sAjaxSource": "tbl_regenviolab.php", // Load Data
	"sServerMethod": "POST",
    "language": {
    ...
    'columnDefs': [
      ...
      {"orderable": false, "targets": 3, "searchable": true, "class": "small font-weit"},
      {"orderable": false, "targets": 4, "searchable": true, "class": "small"},
