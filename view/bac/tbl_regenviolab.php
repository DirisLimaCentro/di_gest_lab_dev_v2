...
$rResult = $bac->get_tblDatosLabReferencialNoEnviada($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $bac->get_tblCountLabReferencialNoEnviada($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $bac->get_tblCountLabReferencialNoEnviada($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $row = array($idcod, $aRow['nro_reglab'] ."-".$aRow['anio_reglab'], $aRow['nro_atencion'] ."-".$aRow['anio_atencion'], $aRow['nombre_rspac'], $aRow['abrev_tipodocpac'] . ": " . $aRow['nro_docpac'], $aRow['nro_hc'], $aRow['dependencia_solitante'],$btnDet);
    $output['aaData'][] = $row;
}
