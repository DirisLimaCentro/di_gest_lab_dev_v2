...
$rResult = $ba->get_tblDatosEnvioALabReferencial($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ba->get_tblCountEnvioALabReferencial($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ba->get_tblCountEnvioALabReferencial($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnDet = '<button class="btn btn-success btn-xs" onclick="open_detalle(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></button>';
	
	$row = array($aRow['nro_envio'], $aRow['fec_envio'], "<span class=\"badge bg-blue\">".$aRow['cnt_solienvtot']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_finalizado']."</span>", $aRow['nom_estadoreg'], $btnDet);
    $output['aaData'][] = $row;
}
