...
$aColumns = array('rbac.nro_reglab', '', '', '', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'rbac.nro_reglab';
...
if (!empty($aOrderingRules)) {
    ...
    $sOrder = " Order By rbac.nro_reglab, rbac.anio_reglab";
}
...
$param[0]['id_tipprocedimiento'] = $input['id_tipprocedimiento'];
...
$rResult = $ba->get_tblDatosResultadoLabEESS($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ba->get_tblCountResultadoLabEESS($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ba->get_tblCountResultadoLabEESS($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnDet = '<button class="btn btn-success btn-xs" onclick="open_pdfsolisinvalor(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></button>';
    //Aqui agrego segundo boton
	$btnResul = '<button class="btn btn-primary btn-xs" onclick="reg_resultado(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-ok"></i></button>';


    $row = array($aRow['nro_reglab']."-".$aRow['anio_reglab'], $aRow['nombre_rspac'], $aRow['abrev_tipodocpac'].": ".$aRow['nro_docpac'], $aRow['nro_hc'], $aRow['dependencia_solitante'], "<b>" . $aRow['estado_resultado'] . "<b>", $aRow['resultado'], $btnDet . " " . $btnResul);
    $output['aaData'][] = $row;
}
