...
$rResult = $ba->get_tblDatosSolicitud($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ba->get_tblCountSolicitud($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ba->get_tblCountSolicitud($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $row = array($aRow['nro_atencion']."-".$aRow['anio_atencion'], $aRow['abrev_tipodocpac'].": ".$aRow['nro_docpac'], $aRow['nro_hc'], $aRow['nombre_rspac'], $aRow['nombre_rssoli'], $aRow['fec_atencion'], $aRow['fec_finalizado'], $aRow['nom_estadosolibac'], $btnDet);
    $output['aaData'][] = $row;
}
