...
$aColumns = array('pape.create_finalize_at', 'pape.create_finalize_at', '', '', '', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'pape.create_finalize_at';
...
if (!empty($aOrderingRules)) {
    ...
    $sOrder = " Order By pape.create_finalize_at desc";
}
...
foreach ($rResult as $aRow) {
    ...
    if ($tipoFin == 0){
      ...
      $btnFor = '<button class="btn btn-danger btn-xs" onclick="event.preventDefault(); open_noconfor(\'' . $aRow['id_papenvio'] . '\');"><i class="glyphicon glyphicon-open"></i></button>';
    }
    ...
    $row = array($btnfechaEnv, $aRow['fec_papfinalizado'], $aRow['nom_depen'], $aRow['nro_papenv'], "<span class=\"badge bg-blue\">".$aRow['cnt_solienvtot']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_soliaceptada']."</span>", "<span class=\"badge bg-red\">".$aRow['cnt_solirechazada']."</span>", "<span class=\"badge bg-blue\">".$aRow['cnt_soliprocesada']."</span>", "<span class=\"badge \">".$aRow['cnt_solinoconfor']."</span>", $btnDet.$btnFor.$btnImpriResult);
    $output['aaData'][] = $row;
}
