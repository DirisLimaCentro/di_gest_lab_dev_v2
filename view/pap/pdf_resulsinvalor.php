...
$idSolicitud = $_GET['id_solicitud'];
$rsSPAPR = $pap->get_datosResultado($idSolicitud);
/*print_r($rsSPAPR);
exit();*/
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(90, 4, utf8_decode($rsSPAP[0]['nom_depen']),'TBR',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(117, 4, utf8_decode($rsSPAP[0]['nombre_rs']),0,0,'L', false);
$pdf->SetFont('Arial','B' ,8);
...
$dir = $rsSPAP[0]['departamento'] . "-" . $rsSPAP[0]['provincia'] . "-" . $rsSPAP[0]['distrito'] . " " . $rsSPAP[0]['descrip_dir'] . $ref;
$pdf->SetFont('Arial','' ,8);
...
$pdf->SetFont('Arial','B',10);
$pdf->SetX(162);
$pdf->Cell(40, 4, $rsSPAP[0]['nro_ordenatencion']."-".$rsSPAP[0]['anio_ordensoli'],'LBR',1,'C', false);
$pdf->Ln(2);
$pdf->SetFont('Arial','B',11);
...
$pdf->Cell(25, 4, $rsSPAP[0]['nro_hc'],'TBR',1,'L', false);
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(100, 4, utf8_decode($rsSPAP[0]['nombre_rs']),'TBR',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(23, 4, 'EDAD:','LTB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(29, 4, $rsSPAP[0]['edad_pac'],'TBR',1,'L', false);
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(10, 4, number_format($rsSPAP[0]['peso_pac'], 2, '.', ''),'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(18, 4, utf8_decode('TALLA (cm):'),'TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(11, 4, number_format($rsSPAP[0]['talla_pac'], 2, '.', ''),'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(19, 4, utf8_decode('P.A. (mmHg):'),'TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(15, 4, $rsSPAP[0]['pa_pac'],'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(8, 4, utf8_decode('FUR:'),'TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(16, 4, $rsSPAP[0]['fec_fur'],'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(10, 4, 'GEST:','TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(8, 4, $rsSPAP[0]['nro_gest'],'BT',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(10, 4, 'PARA:','TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(17, 4,  $rsSPAP[0]['nro_parapri'] . "-". $rsSPAP[0]['nro_parase'] . "-". $rsSPAP[0]['nro_parater'] . "-". $rsSPAP[0]['nro_paracua'] ,'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(17, 4, 'GESTANTE:','TB',0,'L', false);
$gestante = ($rsSPAP[0]['check_gestante'] == "t") ? 'SI(X)  NO( )' : 'SI( )  NO(X)';
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(18, 4, $gestante,'TBR',1,'L', false);
...
$rsSPAPA = $pap->get_datosAnticonceptivoPorIdSolicitud($idSolicitud);
if (count($rsSPAPA) <> 0){
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(161, 4, utf8_decode('NINGUNO' . $anti5 . '   DIU' . $anti1 . '   ORAL' . $anti2 . '   INYEC' . $anti3 . '   IMPLANTE' . $anti4 . '   OTRO: ' . $anti6),'TBR',1,'L', false);

$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(41, 4, utf8_decode('TAMIZAJE ANTERIOR:'),'LTB',0,'L', false);
//$papante = ($rsSPAP[0]['id_tamizajeante'] <> "0") ? 'SI(X)  NO( )' : 'SI( )  NO(X)';
$papante = $rsSPAP[0]['nom_tamizaje'];
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(27, 4, $papante,'TB',0,'L', false);
$pdf->SetFont('Arial','B' ,8);
$pdf->Cell(25, 4, 'RESULTADO:','TB',0,'L', false);
$pdf->SetFont('Arial','' ,8);
if($rsSPAP[0]['id_resultamiante'] <> ""){
  $resulPosi = ($rsSPAP[0]['id_resultamiante'] == "1") ? '(X)' : '( )';
  $resulNega = ($rsSPAP[0]['id_resultamiante'] == "2") ? '(X)' : '( )';
...
$pdf->SetFont('Arial','' ,8);
if($rsSPAP[0]['anio_tamizajeante'] <> ""){
  $pdf->Cell(31, 4, $rsSPAP[0]['anio_tamizajeante'],'TBR',1,'L', false);
} else {
  $pdf->Cell(31, 4, '--','TBR',1,'L', false);
}

if($rsSPAP[0]['id_tamizajeante'] == "1" OR $rsSPAP[0]['id_tamizajeante'] == "2"){

  $esca1 = ($rsSPAP[0]['id_anorescamosaante'] == "1") ? '(X)' : '( )';
  $esca2 = ($rsSPAP[0]['id_anorescamosaante'] == "2") ? '(X)' : '( )';
  $esca3 = ($rsSPAP[0]['id_anorescamosaante'] == "3") ? '(X)' : '( )';
  $esca4 = ($rsSPAP[0]['id_anorescamosaante'] == "4") ? '(X)' : '( )';
  $esca5 = ($rsSPAP[0]['id_anorescamosaante'] == "5") ? '(X)' : '( )';
  $esca6 = ($rsSPAP[0]['id_anorescamosaante'] == "5") ? '(X)' : '( )';
  $pdf->SetFont('Arial','' ,8);
  $pdf->Cell(0, 4, utf8_decode('  ASCUS '.$esca1.'   L.I.E. de bajo grado '.$esca2.'   ASCH '.$esca3.'   L.I.E. de alto grado '.$esca4.'   CARCINOMA IN SITU '.$esca5.'   CARCINOMA INVASOR '.$esca6),'LBR',1,'L', false);

  $glandu1 = ($rsSPAP[0]['id_anorglandularante'] == "1") ? '(X)' : '( )';
  $glandu2 = ($rsSPAP[0]['id_anorglandularante'] == "2") ? '(X)' : '( )';
  $glandu3 = ($rsSPAP[0]['id_anorglandularante'] == "3") ? '(X)' : '( )';
  $glandu4 = ($rsSPAP[0]['id_anorglandularante'] == "4") ? '(X)' : '( )';
  $pdf->SetFont('Arial','' ,8);
  $pdf->Cell(64, 4, utf8_decode('  CÉLULAS GLANDULARES ATÍPICAS '.$glandu1),'L',0,'L', false);
  $pdf->Cell(130, 4, utf8_decode('  CÉLULAS GLANDULARES ATÍPICAS SUGESTIVAS DE NEOPLASIA '.$glandu2),'R',1,'L', false);
  $pdf->Cell(64, 4, utf8_decode('  ADENOCARCINOMA IN SITU '.$glandu3),'LB',0,'L', false);
...
$rsSPAPS = $pap->get_datosSintomaPorIdSolicitud($idSolicitud);
if (count($rsSPAPS) <> 0){
...
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(159, 4, utf8_decode('NINGUNO' . $sinto1 . '   DOLOR' . $sinto2 . '   LEUCORREA' . $sinto3 . '   PRURITO' . $sinto4 . '   COITORRAGIA' . $sinto5 . '   OTRO: ' . $sinto6),'TBR',1,'L', false);
...
$rsSPAPE = $pap->get_datosExaCervicoPorIdSolicitud($idSolicitud);
if (count($rsSPAPE) <> 0){
...
$rsPAPD = $pap->get_datosDiagnosticoPorIdSolicitud($idSolicitud);
if (count($rsPAPD) <> 0){
  foreach ($rsPAPD as $rowD) {
    if ($rowD['id_diagnostico'] == "Z01.4") $diag1 = '- EXAMEN GINECOLÓGICO (GENERAL) (DE RUTINA)';
    if ($rowD['id_diagnostico'] == "N72") $diag1 = '- N72: CERVICITIS';
    if ($rowD['id_diagnostico'] == "N84.1") $diag2 = '- N84.1: POLIPO DEL CUELLO UTERINO';
    if ($rowD['id_diagnostico'] == "N86") $giag3 = '- N86: EROSIÓN Y ECTROPIÓN';
  }
}
$pdf->SetFont('Arial','' ,8);
$pdf->Cell(0, 4, utf8_decode(trim($diag1."     ".$diag2."     ".$giag3)),'LBR',1,'L', false);

$pdf->Image('examen/'.$rsSPAP[0]['id_papsoli'].'.png',95,$ejeYimgExa,44);

$pdf->Ln(5);
$url = "../genecrud/profesional/";
$nomArchiJpg = $rsSPAP[0]['id_profesional'].".jpg";
if (file_exists($url . $nomArchiJpg)) {
  $pdf->Image($url.$nomArchiJpg,162,$pdf->GetY(),25);
}
$nomArchiPng = $rsSPAP[0]['id_profesional'].".png";
if (file_exists($url . $nomArchiPng)) {
  $pdf->Image($url.$nomArchiPng,162,$pdf->GetY(),25);
}
