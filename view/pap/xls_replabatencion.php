...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', ' REPORTE DE REGISTRO DE CITOLOGÍA');
...
$objPHPExcel->getActiveSheet()->getStyle('A1:O1')->applyFromArray($styleTituloPrincipal);
$objPHPExcel->getActiveSheet()->getStyle('A2:O2')->applyFromArray($styleTituloPrincipal);

$objPHPExcel->setActiveSheetIndex(0)
->setCellValue('A4', utf8_encode('Item'))
->setCellValue('B4', utf8_encode('Establecimiento'))
->setCellValue('C4', utf8_encode('Fec. Recepcion'))
->setCellValue('D4', 'Doc. Identidad')
->setCellValue('E4', utf8_encode('Nro. Documento'))
->setCellValue('F4', 'HC')
->setCellValue('G4', 'SIS')
->setCellValue('H4', 'Paciente')
->setCellValue('I4', 'Edad')
->setCellValue('J4', utf8_encode('Resultado'))
->setCellValue('K4', utf8_encode('Bethesda'))
->setCellValue('L4', utf8_encode('Nro. Reg. Lab.'))
->setCellValue('M4', 'Año')
->setCellValue('N4', 'Fec. Resultado')
->setCellValue('O4', 'Fec. Finalizado');
$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacionTitulo, "A4:O4");

$param[0]['fecIniAte'] = $_GET['fecIni'];
$param[0]['fecFinAte'] = $_GET['fecFin'];
$param[0]['tipoFec'] = $_GET['tipoFec'];
$param[0]['idProfe'] = $_GET['idProfe'];

$rs = $pap->get_repDatosPAPLaboratorio($param);
...
if ($nr > 0) {
  foreach ($rs as $row) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $max, $row['abrev_tipodocpac']);//tipo doc
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $max, $row['nro_docpac']); //nro doc//$rsPer[0]['nro_documento'], PHPExcel_Cell_DataType::TYPE_STRING);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $max, $row['nro_hc']); //hc//$rsPer[0]['nro_documento'], PHPExcel_Cell_DataType::TYPE_STRING);
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $max, $row['nom_resul']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K' . $max, $row['nom_bethesa']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L' . $max, $row['nro_reglab']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M' . $max, $anioLab);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N' . $max, $row['fec_resul']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O' . $max, $row['fec_papfinal']);
