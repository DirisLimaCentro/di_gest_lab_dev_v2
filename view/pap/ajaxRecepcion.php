...
if($_SESSION['id_rol']!='6'){ $validrol=" and tm.usu_recibe='".$_SESSION['id_usuario']."' "; }else{ $validrol= "";}
	if (!empty($aFilteringRules)) {
		$sWhere = " WHERE  t.idestado_ht='1' and idestado_mov = '1' ".$validrol." and tm.dep_recibe
	...
	*/
	...
	$rResult=$exp->QryRecepcion($sWhere,$sOrder,$sLimit);

	$rResultFilterTotal=$exp->countRecepcion($sWhere);

	list($iFilteredTotal) = $rResultFilterTotal;

	$rResultTotal=$exp->countRecepcion($sWhere);
	...
	*/
	...
	// Looping Data
		...
		if ($aRow["idestado_mov"]=='1'){ $ebien= "<i class='fa fa-check text-success text-center'></i>";}else { $ebien= "<i class='fa fa-clock-o text-warning text-center'></i>";}
		$btn = '<a href="#" onClick="ShowRuta(\''.$idcod.'\')"><i class="fa fa-eye"></i></a>';
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			$row[] = $aRow[ $aColumns[$i] ];
