...
foreach ($rResult as $aRow) {
    ...
    if(($aRow['idestado_envdet'] == "3")  OR ($aRow['idestado_envdet'] == "5")){
      ...
      $btnDetSub = ' <a href="#" data-target="#showSubModal" class="acept" data-toggle="tooltip" data-placement="top" title="Subsanar observación" onclick="event.preventDefault(); open_subsanar(\'' . $aRow['id_papenviodet'] . '\', \'' . $aRow['nro_ordensoli']."-".$aRow['anio_ordensoli'] . '\');"><i class="glyphicon glyphicon-ok"></i></a>';
    ...
    $btnDetObs = ' <a href="#" data-target="#editProductModal" data-toggle="tooltip" data-placement="top" title="Ver observación" onclick="event.preventDefault(); open_detobs(\'' . $aRow['id_papenviodet'] . '\');"><i class="glyphicon glyphicon-info-sign"></i></a>';
    ...
    $row = array($aRow['nro_reglab'], $aRow['nro_ordensoli']."-".$aRow['anio_ordensoli'], $aRow['abrev_rspac'].$priNomFalle.$otroNomFalle, $aRow['nombre_rspac'], $aRow['abrev_tipodocpac'] . ": " . $aRow['nro_docpac'], $aRow['nro_hc'], $aRow['nom_sispac'], $aRow['edad_pac'], $nomEstado, $btnDet.$btnDetObs.$btnDetSub.$btnDetRec);
    $output['aaData'][] = $row;
}
