...
$aColumns = array('g.descrip_grupo', 'g.estado', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'g.descrip_grupo';
...
if (!empty($aOrderingRules)) {
    ...
    $sOrder = " Orde By g.descrip_grupo";
}
...
$rResult = $g->get_tblDatosGrupo($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $g->get_tblCountGrupo($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $g->get_tblCountGrupo($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnEdit = '<button class="btn btn-success btn-xs" onclick="open_editar(\'' . $aRow['id_grupo'] . '\');"><i class="glyphicon glyphicon-pencil"></i></button>';
    ...
    $row = array($aRow['descrip_grupo'], $nomEstado, $btnEdit);
    $output['aaData'][] = $row;
}
