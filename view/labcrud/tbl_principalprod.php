...
foreach ($rResult as $aRow) {
    ...
    $btnDep = '<button class="btn btn-primary btn-xs" onclick="open_dependencia(\'' . $aRow['id_producto'] . '\',\'' . $aRow['nom_producto'] . '\');"><i class="fa fa-hospital-o"></i></button>';
    $btnComp = '<button class="btn btn-info btn-xs" onclick="open_comp(\'' . $aRow['id_producto'] . '\',\'' . $aRow['nom_producto'] . '\');"><i class="glyphicon glyphicon-list-alt"></i></button>';
    ...
    $row = array($aRow['codref_producto'], $aRow['nomtipo_producto'], $aRow['nom_producto'], nl2br($aRow['descrip_prepapro']), "<b>".$aRow['cnt_dep']."</b>", "<b>".$aRow['cnt_comp']."</b>", $nomEstado, $btnEdit.$btnDep.$btnComp);
    $output['aaData'][] = $row;
}
