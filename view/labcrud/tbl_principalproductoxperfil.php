...
$aColumns = array('nom_producto', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'nom_producto';
...
$param[0]['nombre'] = $input['nombre'];
//print_r($param);
//Aqui se manda los parametros de busqueda
$rResult = $pe->get_tblDatosProducto($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $pe->get_tblCountProducto($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal =$pe->get_tblCountProducto($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnGrupo = '<button class="btn btn-success btn-xs" onclick="add_componente(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-ok"></i></button>';

    $row = array($aRow['nom_producto'], $btnGrupo);
    $output['aaData'][] = $row;
}
