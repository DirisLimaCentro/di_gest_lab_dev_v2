...
$aColumns = array('a.ord_area', '', '', '', 'a.visible', 'a.estado', ''); //Kolom Pada Tabel
...
$rResult = $a->get_tblDatosArea($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $a->get_tblCountArea($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $a->get_tblCountArea($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnEdit = '<button class="btn btn-success btn-xs" onclick="edit_registro(\'' . $aRow['id_area'] . '\');"><i class="glyphicon glyphicon-pencil"></i></button>';

    if ($aRow['estado'] == "1") {
        $styleEst = "bg-green";
        $btnBajar = '<button class="btn btn-primary btn-xs" onclick="cambiar_orden(\'B\',\'' . $aRow['id_area'] . '|' . $aRow['ord_area'] . '\');"><i class="glyphicon glyphicon-circle-arrow-up"></i></button>';
        $btnSubir = '<button class="btn btn-primary btn-xs" onclick="cambiar_orden(\'S\',\'' . $aRow['id_area'] . '|' . $aRow['ord_area'] . '\');"><i class="glyphicon glyphicon-circle-arrow-down"></i></button>';
    } else {
        $styleEst = "bg-red";
    ...
    $row = array($aRow['ord_area'], $btnBajar." ".$btnSubir, $aRow['abrev_area'],  $aRow['descrip_area'], $aRow['nom_visible'], $nomEstado, $btnEdit);
    $output['aaData'][] = $row;
}
