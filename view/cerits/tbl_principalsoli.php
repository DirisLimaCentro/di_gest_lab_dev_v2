...
$rResult = $ce->get_tblDatosSoliConstanciaITS($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ce->get_tblCountSoliConstanciaITS($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ce->get_tblCountSoliConstanciaITS($sWhere, $param);
...
foreach ($rResult as $aRow) {
  ...
  $btnEdit = '';
  $btnEli = '';
  //$btnEdit = ' <a href="#" data-target="#editProductModal" class="acept" data-toggle="tooltip" data-placement="top" title="Editar toma PAP" onclick="edit_solicitud(\'' . $aRow['id_cerisoliits'] . '\');"><i class="glyphicon glyphicon-pencil"></i></a>';
  $btnDet = ' <a href="#" data-target="#editProductModal" class="detail" data-toggle="tooltip" data-placement="top" title="Ver ficha de atención" onclick="open_pdfsinvalor(\'' . $aRow['id_cerisoliits'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></a>';
  ...
  $row = array($aRow['nro_ordensoli']."-".$aRow['anio_ordensoli'], $aRow['nombre_rspac'], $aRow['abrev_tipodocpac'].": ".$aRow['nro_docpac'], $aRow['nro_hc'], $aRow['nombre_profe'], $aRow['nom_depen'], $aRow['fec_atencion'], $btnEdit.$btnDet.$btnEli);
  $output['aaData'][] = $row;
}
