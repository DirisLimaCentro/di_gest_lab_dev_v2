...
$aColumns = array('pr.cod_refprofesion', '', '', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'pr.cod_refprofesion';
...
if (!empty($aOrderingRules)) {
    ...
    $sOrder = " Orde By pr.cod_refprofesion";
}
...
$rResult = $pr->get_tblDatosProfesion($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $pr->get_tblCountProfesion($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $pr->get_tblCountProfesion($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnEdit = '<button class="btn btn-success btn-xs" onclick="open_editar(\'' . $aRow['id_profesion'] . '\');"><i class="glyphicon glyphicon-pencil"></i></button>';
    ...
    $row = array($aRow['cod_refprofesion'], $aRow['nom_profesion'], $nomEstado, $btnEdit);
    $output['aaData'][] = $row;
}
