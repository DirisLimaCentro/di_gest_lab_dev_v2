...
$aColumns = array('ups.descripcion', '', '', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'ups.descripcion';
...
if (!empty($aOrderingRules)) {
    ...
    $sOrder = " Orde By ups.descripcion";
}
...
$rResult = $ups->get_tblDatosUps($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ups->get_tblCountUps($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ups->get_tblCountUps($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnEdit = '<button class="btn btn-success btn-xs" onclick="open_editar(\'' . $aRow['id_ups'] . '\');"><i class="glyphicon glyphicon-pencil"></i></button>';
    ...
    $row = array($aRow['id_ups'], $aRow['abreviatura'], $aRow['descripcion'], $nomEstado, $btnEdit);
    $output['aaData'][] = $row;
}
