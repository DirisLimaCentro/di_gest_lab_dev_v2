...
$aColumns = array('ca.nom_cargo', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'ca.nom_cargo';
...
$rResult = $ca->get_tblDatosCargo($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $ca->get_tblCountCargo($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $ca->get_tblCountCargo($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnEdit = '<button class="btn btn-success btn-xs" onclick="open_editar(\'' . $aRow['id_cargo'] . '\');"><i class="glyphicon glyphicon-pencil"></i></button>';
    ...
    $row = array($aRow['nom_cargo'], $nomEstado, $btnEdit);
    $output['aaData'][] = $row;
}
