...
function validForm(acc) {
	...
	var txtNroAteManual = $('#txtNroAteManual').val();
	var txtNroAteManualLn = txtNroAteManual.length;
...
function save_atencion(acc) {
        ...
        $.ajax( {
          ...
          + "&txtTipIng=S&accion=POST_ADD_REGSOLICITUDREFERENCIADO",
...
function recargar_pag_reg() {
  window.location = './main_regsolicitudreferenciada.php';
}

function expor_atenciones_hoy(fecha, id_dep) {
	if(id_dep == "67"){
		var urlwindow = "../lab/pdf_repatencion.php?fecIni=" + fecha + "&fecFin=" + fecha;
	} else {
