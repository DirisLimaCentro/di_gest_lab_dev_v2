...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
      <h3 class="panel-title"><strong>REGISTRO DE ENVÍO DE PRODUCTOS REALIZADOS</strong></h3>
    </div>
    <div class="panel-body">
        ...
        <p class="text-right" style="margin: 0 0 0px;"><b>Color por días transcurridos desde la fecha de atención</b>: <span class="label label-success">0 a 3 días</span> <span class="label label-warning">4 a 6 días</span> <span class="label label-danger">7 días a más</span></p>
			<div class="form-group">
				<div class="col-sm-4">
					<label for="txt_bus_id_dependencia">Laboratorio destino:</label>
					<?php $rsD = $d->get_listaDepenInstitucion(); ?>
					<select name="txt_bus_id_dependencia" id="txt_bus_id_dependencia" class="form-control">
				...
				<div class="col-sm-4">
					<label for="txt_bus_id_producto"><small>Examen:</small></label>
					<?php $rsP = $pr->get_listaProductoLaboratorio(); ?>
					<select name="txt_bus_id_producto" id="txt_bus_id_producto" class="form-control" onchange="nueva_busqueda();">
						<option value="0" selected>-- SELECCIONE --</option>
					  <?php
					  foreach ($rsP as $row) {
      ...
      if (result == true) {
        ...
        $.ajax( {
          ...
          data: {
			...
			id_destino: $('#txt_bus_id_dependencia').val(),
			id_producto: $('#txt_bus_id_producto').val(),
...
$(document).ready(function () {
	$("#txt_bus_id_producto, #txt_bus_id_dependencia").select2();
  ...
  var table = $('#example').DataTable({
    "lengthMenu": [[50, 100 ,250], [50, 100 ,250, "All"]],
      ...
      {
        aoData.push({"name": "id_producto", "value": $("#txt_bus_id_producto").val()});
      },
    "language": {
    ...
    'columnDefs': [
      ...
      {"orderable": false, "targets": 2, "searchable": true, "class": "small text-center"},
      {"orderable": false, "targets": 3, "searchable": true, "class": "small font-weit"},
      {"orderable": false, "targets": 4, "searchable": true, "class": "small font-weit"},
