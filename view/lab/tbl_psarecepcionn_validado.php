...
$rResult = $lab->get_tblDatosProductoEnviadosValidados($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$recordsFiltered = 0;
...
foreach ($rResult as $aRow) {
    ...
    $btnImpr = '<button type="button" class="btn btn-warning btn-xs" onclick="imprime_resultado(\'' . md5($aRow['id_atencion']) . '\',\'' . md5($aRow['id_dependencia_origen']) . '\',\'' . $aRow['id_producto'] . '\');"><i class="fa fa-file-text-o"></i></button>';
    $btnResul = '<a href="#" class="info" data-toggle="tooltip" title="Ing. Resultado" onclick="event.preventDefault(); reg_resultado(\'' . $aRow['id_atencion'] . '\',\'' . $aRow['id_producto'] . '\');"><i class="fa fa-share"></i></a>';
    ...
    $output['aaData'][] = array(
        ...
        "fecha_valida_resultado"  => $aRow['fecha_valida_resultado'],
        "id_estado_atencion"        => $aRow['id_estado_atencion'],
        "btn_rechazar"  => $btnImpr, 
        "btn_resultado"  => $btnResul
    );
}
