...
<div class="container-fluid">
  <div class="panel-prime">
    <div class="panel-heading">
      <div class="row">
		<div class="col-sm-6">
			<h3 class="panel-title"><strong>MODIFICAR RESULTADOS VALIDADOS</strong></h3>
		</div>
                  ...
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      ...
                      foreach ($rsCtp as $rowCpt) {
						...
						<?php
						echo "</td>";
						echo "<td class=\"text-center\">" . $btnIng . $btnImpr ."</td>";
                        echo "</tr>";
					  }
                      ?>
                    </tbody>
      ...
      <div class="col-sm-7 col-md-8">
        <div class="panel panel-info">
          ...
          <div class="panel-body">
			...
			<h4 style="margin-bottom: 2px;"><i class="icon fa fa-info"></i> Importante!</h4>
			Seleccione el examen validado que desea editar el resultado "<button type="button" class="btn btn-success btn-xs tbn-ing-producto"><i class="glyphicon glyphicon-pencil"></i></button>".
			</div>
              ...
              <div id="parent">
                <table id="fixTable" class="table table-bordered table-hover">
                  ...
                  <tbody>
					...
					$rsCtp = $at->get_datosProductoPorIdAtencion($idAtencion, $id_producto, 'RV');
                              ...
                              if ($rowC['idtipo_ingresol'] == "1"){
                                ...
                                <div class="form-group<?php echo ' '.$valColor ?>" style="margin-bottom: 1px;">
                                  <div class="input-group input-group-sm">
                                    <input type="text" class="form-control input-sm" id="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>" name="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>" placeholder="" onfocus="this.select()" onkeypress="<?php echo $nameFunValida . " " . $nameFunValRefPorcentaje;?>" onblur="<?php echo $nameFunValida . " " . $nameFunValRefPorcentaje;?>" value="<?php echo $valRes;?>"/>
                                ...
                                <div class="form-group" style="margin-bottom: 1px;">
                                  <textarea class="form-control" id="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>" name="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>" rows="3"><?php echo $rowC['det_result'];?></textarea>
                                  <input type="hidden" id="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_idval" name="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_idval" value="<?php echo $idVal;?>"/>
                                  <input type="hidden" id="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_inf" name="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_inf" value="<?php echo $valMin;?>"/>
                                  <input type="hidden" id="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_sup" name="txt_<?php echo $rowP['id_producto']."_".$rowC['id']?>_sup" value="<?php echo $valMax;?>"/>
                                </div>
				...
				<div class="form-group">
					<label for="txt_descrip_edit_valid"><b>Sustento de la edición:</b></label>
					<textarea name="txt_descrip_edit_valid" id="txt_descrip_edit_valid" class="form-control" rows="3" <?php if($id_producto == 0) echo " disabled"?>></textarea>
				</div>
            </form>
          </div>
			...
			<div class="row">
				<div class="col-md-12 text-center">
				  <div id="saveAtencion">
					<div class="btn-group">
					  <button class="btn btn-success btn-lg" id="btn-submit" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Espere" data-done-text="<i class='fa fa-save'></i> Guardar" onclick="save_atencion('ERV')" <?php if($id_producto == 0) echo " disabled"?>><i class="fa fa-save"></i>  Guardar resultados nuevos</button>
					</div>
				  </div>
...
<div class="modal fade" id="mostrar_ayuda" role="dialog" data-backdrop="static">
	...
	<div class="modal-content">
		...
		<div class="modal-body">
			<p class="text-left small" style="margin: 0 0 0px;"><b>Botones de acción</b>:<br/> <button class="btn btn-success btn-xs"><i class="glyphicon glyphicon-pencil"></i></button>=Seleccionar examen para editar resultado validado | <button class="btn btn-warning btn-xs"><i class="fa fa-file-text-o"></i></button>=Imprimir Resultado</p>
			<hr/>
			<h5>Leyenda:</h5>
				<div class="table-responsive">
...
function reg_resultado(idatencion,id_prod) {
	if(id_prod == ""){
		...
		window.location = './main_editresultadovalidprod.php?nroSoli='+idatencion + '&id_prod=' + id_prod;
	}
}
...
function save_atencion(opt) {
  ...
  if($('#txt_descrip_edit_valid').val().trim().length < 4){
    msg += "Ingrese sustento de la edición del resultado validado<br/>";
    sw = false;	  
  }
    ...
    callback: function (result) {
      if (result == true) {
        ...
        $.ajax( {
		  ...
		  + "&descrip_edit_valid=" + $('#txt_descrip_edit_valid').val() + "&accion=POST_EDIT_RESULTADOLABVALIDADO",
          success: function(data) {
            ...
            if(tmsg == "OK"){
				...
				var id = $('#txtIdAtencion').val();
				window.location = './main_editresultadovalidprod.php?nroSoli='+id;
				/*
              var AnameInput = $('#frmArea').serializeArray();
              var ing = "";
              len = AnameInput.length;
