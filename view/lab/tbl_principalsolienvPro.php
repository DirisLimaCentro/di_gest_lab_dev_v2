...
foreach ($rResult as $aRow) {
	...
	if($input['opt'] == "LAB"){
		if($input['id_estado'] == "2"){
			$btnDet = '<a href="#" data-target="#editProductModal" class="primary" data-toggle="tooltip" data-placement="top" title="Ver lista de muestras" onclick="open_detalle(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></a>';
			...
			$row = array($aRow['fec_recepcion'], $aRow['dependencia_origen'], $aRow['nro_envio']."-".$aRow['anio_envio'], $aRow['estado_env'], "<span class=\"badge bg-blue\">".$aRow['cnt_enviado']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_soliaceptada']."</span>", "<span class=\"badge bg-red\">".$aRow['cnt_solirechazada']."</span>", "<span class=\"badge bg-blue\">".$aRow['cnt_soliprocesada']."</span>", $btnDet.$btnRec);
		} else {
			$btnDet = '<a href="#" data-target="#editProductModal" class="primary" data-toggle="tooltip" data-placement="top" title="Ver lista de muestras" onclick="open_detalle(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></a>';
			$row = array($aRow['fec_recepcion'], $aRow['fec_finaliza'], $aRow['dependencia_origen'], $aRow['nro_envio']."-".$aRow['anio_envio'], "<span class=\"badge bg-blue\">".$aRow['cnt_enviado']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_soliaceptada']."</span>", "<span class=\"badge bg-red\">".$aRow['cnt_solirechazada']."</span>", "<span class=\"badge bg-blue\">".$aRow['cnt_soliprocesada']."</span>", $btnDet.$btnRec);
		}
	} else {
		if($input['id_estado'] == "2"){
			...
			$row = array($aRow['fec_recepcion'], $aRow['nom_producto'], $aRow['nro_envio']."-".$aRow['anio_envio'], $aRow['estado_env'], "<span class=\"badge bg-blue\">".$aRow['cnt_enviado']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_soliaceptada']."</span>", "<span class=\"badge bg-red\">".$aRow['cnt_solirechazada']."</span>", "<span class=\"badge bg-blue\">".$aRow['cnt_soliprocesada']."</span>", $btnRec.$btnDet);		
		} else {
			$btnPrintValid = ' <button type="button" class="btn btn-warning btn-xs" title="Imprimir validados" onclick="print_validados(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-print"></i></button>';
			if($aRow['cnt_solirechazada'] <> "0"){
				$btnRec = ' <button type="button" class="btn btn-danger btn-xs" title="Ver observados" onclick="open_detobs(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-list"></i></button>';
			...
			$btnDet = ' <button type="button" class="btn btn-success btn-xs" title="Ver lista de muestras" onclick="open_detalle(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eye-open"></i></button>';
			$row = array($aRow['fec_recepcion'], $aRow['fec_finaliza'], $aRow['nom_producto'], $aRow['nro_envio']."-".$aRow['anio_envio'], "<span class=\"badge bg-blue\">".$aRow['cnt_enviado']."</span>", "<span class=\"badge bg-green\">".$aRow['cnt_soliaceptada']."</span>", "<span class=\"badge bg-red\">".$aRow['cnt_solirechazada']."</span>", "<span class=\"badge bg-blue\">".$aRow['cnt_soliprocesada']."</span>", $btnRec.$btnDet);
		}
	}
    $output['aaData'][] = $row;
}
