...
/*
  ...
  */
  if($aRow['id_estado_resul'] == "1"){
	  if($aRow['nom_estadoreg'] <> "ANULADO"){
		//$btnResul = '<button class="btn btn-primary btn-xs" onclick="reg_resultado(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eject"></i></button>';
	  }
  }
  if($aRow['id_estado_resul'] == "2" OR $aRow['id_estado_resul'] == "3"){
	//$btnEdit = '<button class="btn btn-primary btn-xs" onclick="open_edit(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-eject"></i></button>';
	
	if($aRow['id_estado_resul'] == "3"){
		if ($labIdRolUser == "1" Or $labIdRolUser == "5" Or $labIdRolUser == "2"){
			//$btnValid = '<button class="btn btn-primary btn-xs" onclick="open_editvalid(\'' . $aRow['id'] . '\');"><i class="glyphicon glyphicon-check"></i></button>';
		}
	}
  }
...
/*
  if($aRow['id_estadoresul'] <> "1"){
    $btnLab = '<button class="btn btn-warning btn-xs" onclick="print_resul(\'' . md5($aRow['id_atencion']) . '\',\'' . md5($aRow['id_dependencia']) . '\',\'0\',\'' . $nomPac . '\');"><i class="fa fa-file-text-o"></i></button>';
	//$btnLab = '<button class="btn btn-warning btn-xs" onclick="imprime_resultado(\'' . md5($aRow['id_atencion']) . '\',\'' . md5($aRow['id_dependencia']) . '\');"><i class="fa fa-file-text-o"></i></button>';
  }
  
  if ($correTipoPlanDep == 1){
    if($aRow['nro_atencion'] == ""){
      $nroTipoPlanDep = " S/C - ";
    } else {
      $nroTipoPlanDep = $aRow['nro_atencion']." - ";
    }
  }
*/
  $row = array($aRow['id'], $aRow['nro_atencion']."-".$aRow['anio_atencion'], $aRow['abrev_plan'], $aRow['nombre_rs'], $aRow['abrev_tipodoc'].": ".$aRow['nrodoc'],  $aRow['nro_hc'], $aRow['fec_atencion'], $aRow['nom_estadoreg'], $aRow['nom_estadoresul'], $btnFUA.$btnLab.$btnResul.$btnEdit.$btnValid);
  $output['aaData'][] = $row;
}
