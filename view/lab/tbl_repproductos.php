...
$param[0]['id_establecimiento'] = $input['id_establecimiento'];
$param[0]['nom_producto'] = $input['nom_producto'];
//print_r($param);
//Aqui se manda los parametros de busqueda
$rResult = $pr->get_tblDatosProductoDependencia($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $pr->get_tblCountProductoDependencia($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $pr->get_tblCountProductoDependencia($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $btnComp = '<button class="btn btn-info btn-xs" onclick="open_comp(\'' . $aRow['id_producto'] . '\',\'' . $nom_producto . '\',\'' . $aRow['id_dependencia'] . '\');"><i class="glyphicon glyphicon-list-alt"></i></button>';
    ...
    $row = array($aRow['nom_depen'], $aRow['codref_producto'], $aRow['nomtipo_producto'], $aRow['nom_producto'], nl2br($aRow['descrip_prepapro']), $aRow['prec_sis'], $aRow['prec_parti'], "<b>".$aRow['cnt_comp']."</b>", $nomEstado, $btnComp);
    $output['aaData'][] = $row;
}
