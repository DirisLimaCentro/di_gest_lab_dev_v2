...
function obtener_nroatencion(fecha, opt = 1){
...
function busca_calendariocita_mes_anio(){
  $.ajax({
    ...
    type: 'POST',
    data: 'accion=SHOW_CALENDARIOPORMESYANIO&anio=' + $('#txtBusAnio').val() + '&mes=' + $('#txtBusMes').val() + '&origen=REG',
...
function validForm(acc) {
	...
	if (txtFecNacPac == "") { msg += "Ingrese FECHA DE NACIMIENTO del paciente<br/>"; sw = false;}
    ...
    callback: function (result) {
      if (result == true) {*/
        ...
        $.ajax( {
          ...
          + "&txtTipIng=S&accion=POST_ADD_REGSOLICITUD",
          success: function(data) {
