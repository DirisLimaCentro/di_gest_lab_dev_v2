...
$rResult = $lab->get_tblDatosProductoAtendidas($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
...
foreach ($rResult as $aRow) {
    ...
    if ($aRow['id_estado_atencion'] <> "5"){
        $btnEntre = '<button type="button" class="btn btn-primary btn-xs" onclick="acc_registro(\'' . $aRow['id_atencion'] . '\',\'' . $aRow['nro_atencion'] . '\',\'EP\');"><i class="glyphicon glyphicon-user"></i></button>';
    }
    ...
    $output['aaData'][] = array(
        ...
        "estado_atencion"=> $aRow['estado_atencion'],
        "btn_resultado"  => trim($btnImpr . " " . $btnEntre)
    );
}
