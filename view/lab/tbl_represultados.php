...
$aColumns = array('la.nro_atenciondia', 'p.nrodoc', '', 'la.fec_atencion', 'la.create_resul', '', ''); //Kolom Pada Tabel
...
$param[0]['id_examen'] = $input['id_examen'];
$param[0]['idDepAten'] = $labIdDepUser;

//print_r($param);
//Aqui se manda los parametros de busqueda
$rResult = $at->get_tblRepDatosResultadoExamenPorFecha($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $at->get_tblRepCountResultadoExamenPorFecha($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $at->get_tblRepCountResultadoExamenPorFecha($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $row = array($aRow['nro_atenciondia'], $aRow['abrev_tipodoc'].": ".$aRow['nrodoc'],  $aRow['nombre_rs'], $aRow['abrev_plan'], $aRow['fec_atencion'], $aRow['fec_resultado'], $aRow['componente'], $aRow['det_resul'], trim($aRow['uni_medida']));
    $output['aaData'][] = $row;
}
