...
$aColumns = array('la.nro_atencion', 'p.nrodoc', '', 'la.fec_atencion', '', '', ''); //Kolom Pada Tabel
// Indexed column (used for fast and accurate table cardinality)
$sIndexColumn = 'p.nrodoc';
...
$param[0]['condicion_urgente'] = $input['condicion_urgente'];
$param[0]['chk_dep_procedencia'] = (empty($input['condicion_dep_procedencia'])) ? "" : "1";
$param[0]['id_dep_procedencia'] = '';
if(!empty($param[0]['chk_dep_procedencia'])){
	$param[0]['id_dep_procedencia'] = $input['id_dep_procedencia'];
}

//print_r($param);
//Aqui se manda los parametros de busqueda
$rResult = $at->get_tblRepDatosAtencion($sWhere, $sOrder, $sLimit, $param);
//print_r($rResult);
$rResultFilterTotal = $at->get_tblRepCountAtencion($sWhere, $param);
/* echo $rResultFilterTotal."Jose";
  exit(); */
list($iFilteredTotal) = $rResultFilterTotal;
$rResultTotal = $at->get_tblRepCountAtencion($sWhere, $param);
...
foreach ($rResult as $aRow) {
    ...
    $row = array($nro_atencion.$es_referencia, $aRow['abrev_tipodoc'].": ".$aRow['nrodoc'],  $aRow['nombre_rs'], $aRow['fec_atencion'], $aRow['abrev_plan'], $aRow['nom_estadoresul'], $btnRUA.$btnLab);
    $output['aaData'][] = $row;
}
